...
struct CardSwapView: View {
    ...
    @AppStorage("nickname") var nickname: String = "" // 프로퍼티로 선언
    ...
    var onSave: (Learner) -> Void // check 버튼
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Image(cardChange ? (manager.receivedLearner?.imageName ?? "Stella") : nickname) // 상태에 따라 이미지 바뀜
                .resizable()
            ...
            Text(!manager.isConnected ? "주변에 있는 러너들을 찾고 있어요!" : cardChange ? "아카데미 러너 \(manager.receivedLearner?.imageName ?? "")를 얻었다!" : "\(manager.receivedLearner?.imageName ?? "")를 발견했어!\n스와이프로 상대에게 카드를 전송해!")
